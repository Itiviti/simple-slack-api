...
class SlackWebSocketSessionImpl extends AbstractSlackSessionImpl implements SlackSession, MessageHandler.Whole<String> {
    ...
    private final PresenceChangeListener internalPresenceChangeListener = new PresenceChangeListener() {
        @Override
    ...
    private final SlackChannelArchivedListener internalChannelArchiveListener = new SlackChannelArchivedListener() {
        @Override
    ...
    private final SlackChannelCreatedListener internalChannelCreatedListener = new SlackChannelCreatedListener() {
        @Override
    ...
    private final SlackChannelDeletedListener internalChannelDeletedListener = new SlackChannelDeletedListener() {
        @Override
        public void onEvent(SlackChannelDeleted event, SlackSession session) {
            channels.remove(event.getSlackChannel().getId());
        }
    };
    private final SlackChannelRenamedListener internalChannelRenamedListener = new SlackChannelRenamedListener() {
        @Override
    ...
    private final SlackChannelUnarchivedListener internalChannelUnarchivedListener = new SlackChannelUnarchivedListener() {
        @Override
    ...
    private final SlackTeamJoinListener internalTeamJoinListener = new SlackTeamJoinListener() {
        @Override
        public void onEvent(SlackTeamJoin event, SlackSession session) {
            users.put(event.getUser().getId(), event.getUser());
        }
    };
    private final SlackUserChangeListener internalUserChangeListener = new SlackUserChangeListener() {
        @Override
    ...
    private void addInternalListeners() {
        addPresenceChangeListener(internalPresenceChangeListener);
        addChannelArchivedListener(internalChannelArchiveListener);
        addChannelCreatedListener(internalChannelCreatedListener);
        addChannelDeletedListener(internalChannelDeletedListener);
        addChannelRenamedListener(internalChannelRenamedListener);
        addChannelUnarchivedListener(internalChannelUnarchivedListener);
        addSlackTeamJoinListener(internalTeamJoinListener);
        addSlackUserChangeListener(internalUserChangeListener);
    }
    ...
    public void connect() throws IOException {
        ...
        connectImpl();
        log.debug("starting actions monitoring");
    ...
    public void disconnect() {
        wantDisconnect = true;
        log.debug("Disconnecting from the Slack server");
        disconnectImpl();
    ...
    private void connectImpl() throws IOException {
        ...
        if (sessionParser.getError() != null) {
            log.error("Error during authentication : {}", sessionParser.getError());
        ...
        log.info("Team " + team.getId() + " : " + team.getName());
        log.info("Self " + sessionPersona.getId() + " : " + sessionPersona.getUserName());
        log.info("{} users found on this session", users.size());
        log.info("{} channels found on this session", channels.size());
        webSocketConnectionURL = sessionParser.getWebSocketURL();
        log.debug("retrieved websocket URL : {}", webSocketConnectionURL);
    ...
    private void establishWebsocketConnection() throws IOException {
        ...
        log.debug("initiating actions to websocket");
        ...
        try {
            websocketSession = client.connectToServer(new Endpoint() {
                ...
                public void onError(Session session, Throwable thr) {
                    log.error("Endpoint#onError called", thr);
                }
        ...
        if (websocketSession != null) {
            ...
            log.debug("websocket actions established");
            log.info("slack session ready");
        } else {
    ...
    private String consumeToString(InputStream content) throws IOException {
        Reader reader = new InputStreamReader(content, StandardCharsets.UTF_8.name());
        StringBuilder builder = new StringBuilder();
        char[] data = new char[16384];
        int numread;
        while (0 <= (numread = reader.read(data)))
            builder.append(data, 0, numread);
                    ...
                    try {
                        ...
                        if (lastPingSent != lastPingAck || websocketSession == null) {
                            ...
                            try {
                                ...
                                log.error("exception while trying to close the websocket ", e);
                            }
                            websocketSession = null;
                            ...
                            log.debug("sending ping {}", lastPingSent);
                            try {
                                ...
                                log.warn("exception caught while using websocket ", e);
                        ...
                        log.error("unexpected exception on monitoring thread ", e);
                    }
                }
    ...
    public SlackMessageHandle<SlackChannelReply> openMultipartyDirectMessageChannel(SlackUser... users) {
        ...
        if (!handle.getReply().isOk()) {
            log.debug("Error occurred while performing command: '" + handle.getReply().getErrorMessage() + "'");
            return null;
        }
        return handle;
    }
    ...
    private void postSlackCommand(Map<String, String> params, String command, SlackMessageHandle handle) {
        ...
        try {
            request.setEntity(new UrlEncodedFormEntity(nameValuePairList, StandardCharsets.UTF_8.name()));
            HttpResponse response = client.execute(request);
            String jsonResponse = consumeToString(response.getEntity().getContent());
            log.debug("PostMessage return: {}", jsonResponse);
    ...
    private void postSlackCommandWithFile(Map<String, String> params, byte[] fileContent, String fileName, String command, SlackMessageHandle handle) {
        ...
        try {
            ...
            HttpResponse response = client.execute(request);
            String jsonResponse = ReaderUtils.readAll(new InputStreamReader(response.getEntity().getContent(), Charset.forName(StandardCharsets.UTF_8.name())));
            log.debug("PostMessage return: {}", jsonResponse);
            ParsedSlackReply reply = SlackJSONReplyParser.decode(parseObject(jsonResponse), this);
            handle.setReply(reply);
        } catch (Exception e) {
    ...
    public void onMessage(String message) {
        ...
        log.debug("receiving from websocket {}", message);
        if (object.get("type") == null) {
            log.info("unable to parse message, missing 'type' attribute: {}", message);
            return;
        }
        if ("pong".equals(object.get("type").getAsString())) {
            lastPingAck = object.get("reply_to").getAsInt();
            log.debug("pong received {}", lastPingAck);
        } else if ("reconnect_url".equals(object.get("type").getAsString())) {
            String newWebSocketConnectionURL = object.get("url").getAsString();
            log.debug("new websocket connection received {}", newWebSocketConnectionURL);
        } else {
        ...
        void dispatch(SlackEvent event) {
            switch (event.getEventType()) {
                ...
                case USER_TYPING:
                    ...
                    break;
                default:
                    log.warn("event of type " + event.getEventType() + " not handled: " + ((UnknownEvent) event).getJsonPayload());
            }
        }
        ...
        private <E extends SlackEvent, L extends SlackEventListener<E>> void dispatchImpl(E event, List<L> listeners) {
            for (L listener : listeners) {
                try {
                    ...
                    log.error("caught exception in dispatchImpl", thr);
                }
            }
        }
    }
