...
class SlackWebSocketSessionImpl extends AbstractSlackSessionImpl implements SlackSession, MessageHandler.Whole<String>
{
    ...
    SlackWebSocketSessionImpl(WebSocketContainerProvider webSocketContainerProvider, String authToken, Proxy.Type proxyType, String proxyAddress, int proxyPort, String proxyUser, String proxyPassword, boolean reconnectOnDisconnection,
    ...
    @Override
    public SlackMessageHandle<SlackMessageReply> sendMessage(SlackChannel channel, JsonObject preparedMessage, SlackChatConfiguration chatConfiguration)
    {
        SlackMessageHandle<SlackMessageReply> handle = new SlackMessageHandle<>(getNextMessageId());
        preparedMessage.addProperty("token", authToken);
        preparedMessage.addProperty("channel", channel.getId());
        if (chatConfiguration.isAsUser())
        {
            preparedMessage.addProperty("as_user", "true");
        }
        if (chatConfiguration.getUserName() != null)
        {
            preparedMessage.addProperty("username", chatConfiguration.getUserName());
        }
        if (chatConfiguration.getAvatar() == Avatar.ICON_URL)
        {
            preparedMessage.addProperty("icon_url", chatConfiguration.getAvatarDescription());
        }
        if (chatConfiguration.getAvatar() == Avatar.EMOJI)
        {
            preparedMessage.addProperty("icon_emoji", chatConfiguration.getAvatarDescription());
        }
        postSlackCommand(preparedMessage, CHAT_POST_MESSAGE_COMMAND, handle);
        return handle;
    }
    ...
    private void postSlackCommand(JsonObject params, String command, SlackMessageHandle handle)
    {
        HttpClient client = getHttpClient();
        HttpPost request = new HttpPost(SLACK_API_HTTPS_ROOT + command);
